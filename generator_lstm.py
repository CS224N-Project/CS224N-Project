...
class LSTMGeneratorModel(object):
    def build(self):
    ...
    def save_preds(self, outFile, metaFile):
        sess = tf.Session()
        saver = tf.train.import_meta_graph('lstm.weights.meta')
    ...
    def fit(self, sess, saver):
        ...
        for epoch in range(self.config.epochs):
            ...
            if dev_mse < best_dev_mse:
                ...
                if saver:
                    print "New best dev MSE! Saving model in ./lstm.weights"
                    # saver.save(sess, './encoder.weights', write_meta_graph = False)
                    saver.save(sess, './lstm.weights')
            print
...
def main(debug=False):
    ...
    with tf.Graph().as_default():
        ...
        generatorModel = LSTMGeneratorModel(config, embedding, train, dev, test, annotations)
